...
class _InitVarMeta(type):
    def __getitem__(cls, tp: Any) -> "InitVar":
        return InitVar(tp)
...
def field(
    *,
    ...
    If kw_only is true, the field will become a keyword-only parameter
    to __init__().
...
def _tuple_str(obj_name: str, fields: Sequence[Field]) -> str:
...
def _init_fn(
    fields: List[Field],
    std_fields: Tuple[Field, ...],
    kw_only_fields: Tuple[Field, ...],
    frozen: bool,
    has_post_init: bool,
    self_name: str,
...
def _is_initvar(a_type: Any, dataclasses: Any) -> bool:
...
def _get_field(cls: type, a_name: str, a_type: Any, default_kw_only: bool) -> Field:
    # Return a Field object for this field name and type.  ClassVars
    # and InitVars are also returned, but marked as such (see
    # f._field_type).
    # default_kw_only is the value of kw_only to use if there isn't a
...
def _get_slots(cls: type) -> List[str]:
...
def _process_class(
    ...
    # initialized here, outside of the "if init:" test, because
    # std_init_fields is used with match_args, below.
    all_init_fields = [
